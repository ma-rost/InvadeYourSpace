...
Sprite::Sprite (const Point <int>& coords, const Point <int>& size):
	spriteSize_ (size) { newCoords (coords); }
...
float Sprite::getResize () { return glb::RESIZE_FACTOR; }

void Sprite::clearCoords () { coordList_.clear(); }
...
void Sprite::setSize (const Point <int>& size) { spriteSize_ = size; }
...
void Sprite::changeSprite () { changeSprite (0, coordList_[0], spriteSize_); }
